...
// !! createAction & createReducer을 createSlice로 리팩토링할 것임. (createSlice제외하고 보려면 아래 주석 해체할 것.)
...
// }
//----------------
//? createSlice는 몇가지 옵션을 받는다: 1. name  2. initialState  3. reducers(action creater를 만들기보다는 action을 직접 넣어보겠음.)
const toDos = createSlice({
  name: "toDoReducer",
  initialState: [],
  reducers: {
    add: (state, action) => {   // addToDo의 명칭 중복 방지를 위해 add로 정정
...
// createSlice는 action, reducer기능 모두 가능함. 따라서 아래처럼 console.log()로 확인해보면 actions, caseReducers, reducer가 담긴 것을 확인할 수 있다.
console.log("toDos", toDos)
// reducer가 주석처리 되었으니 toDos의 reducerfmf export해서 store의 reducer를 다시 설정해줘야 한다.
